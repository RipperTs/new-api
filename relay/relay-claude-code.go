...
func buildFixedClaudeCodeSystem() []claudecode.ClaudeContent {
	return []claudecode.ClaudeContent{
		{
			Type: "text",
		...
		},
		{
			...
			CacheControl: &claudecode.CacheControl{Type: "ephemeral"},
		},
		{
			Type:         "text",
...
func mergeFixedAndUserSystemRaw(userSystemRaw json.RawMessage) (json.RawMessage, error) {
	fixedRaw, err := json.Marshal(buildFixedClaudeCodeSystem())
	if err != nil {
		return nil, err
	}
...
func applyClaudeCodeSystemRules(claudeReq *claudecode.ClaudeRequest, bodyMap map[string]json.RawMessage) (patchedSystemRaw json.RawMessage, shouldPatchSystem bool) {
	...
	if bodyMap == nil {
		...
		if hasCLIKeywordInSystem(claudeReq.System) {
			return nil, false
		}
	...
	if hasCLIKeywordInSystemRaw(userSystemRaw) {
		return nil, false
	}

	claudeReq.System = append(fixedSystem, claudeReq.System...)
	mergedRaw, err := mergeFixedAndUserSystemRaw(userSystemRaw)
	if err == nil {
...
func ClaudeCodeMessagesHelper(c *gin.Context) (openaiErr *dto.OpenAIErrorWithStatusCode) {
	...
	patchedSystemRaw, shouldPatchSystem := applyClaudeCodeSystemRules(&claudeReq, bodyMap)
