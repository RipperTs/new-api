...
func RequestOpenAI2ClaudeMessage(textRequest dto.GeneralOpenAIRequest) (*ClaudeRequest, error) {
	...
	for _, message := range formatMessages {
		if message.Role == "system" {
			...
			if systemText != "" {
				claudeRequest.System = append(claudeRequest.System, ClaudeContent{
					Type:         "text",
					Text:         systemText,
					CacheControl: &CacheControl{Type: "ephemeral"},
				})
...
func ClaudeStreamHandler(c *gin.Context, resp *http.Response, info *relaycommon.RelayInfo, requestMode int) (*dto.OpenAIErrorWithStatusCode, *dto.Usage) {
	...
	// 如果是测试请求，直接返回测试结果
	if isTestRequest {
		testResponse := dto.OpenAITextResponse{
			Id:      responseId,
			Object:  "chat.completion",
			Created: createdTime,
		...
		testResponse.Choices[0].SetStringContent(responseText)

		jsonResponse, err := json.Marshal(testResponse)
		if err != nil {
			return service.OpenAIErrorWrapper(err, "marshal_test_response_failed", http.StatusInternalServerError), nil
		}
		c.Writer.Header().Set("Content-Type", "application/json")
		c.Writer.WriteHeader(200)
		_, err = c.Writer.Write(jsonResponse)
		if err != nil {
			common.LogError(c, "write_test_response_failed: "+err.Error())
		}
		resp.Body.Close()
		return nil, usage
	}
